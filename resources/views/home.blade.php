...
<body class="font-sans antialiased bg-slate-200">
    ...
    <main class="container mx-auto w-full min-h-dvh px-6 pt-5 pb-10">
        <h1 class="text-2xl font-bold ">Produk Toko Nazma</h1>
        <div class="pt-5 grid grid-cols-5 gap-y-5 justify-items-center">
            @foreach ($products as $item )
            <a href="{{route('detail-produk', $item->id)}}">

                <div class="bg-white w-56 h-56 rounded-md cursor-pointer shadow-md overflow-hidden">
                    <div class="h-4/6 overflow-hidden bg-red-300">
                        <img class="object-cover w-full h-full" src="{{ asset('storage/' . $item->path_img) }}" alt="produk_img">
                    </div>
                    <div class="flex-grow h-full px-2 py-3">
                        <h1 class="text-md font-medium truncate">{{$item->name}}</h1>
                        <p class="text-orange-500 font-bold text-xl">Rp {{$item->price}}</p>
                    </div>
                </div>
            </a>
            @endforeach
