...
<body>
    ...
    <main class="container mx-auto w-full min-h-dvh px-56 pt-5 pb-10">
        ...
        <div class="grid grid-cols-2 py-5">
            <div class="h-64 border-orange-400 shadow-md rounded-md overflow-hidden bg-red-300">
                <img class="object-cover w-full h-full" src="{{ asset('storage/' . $product->path_img) }}" alt="produk_img">
            </div>
            <div class="w-full h-64 px-5 py-2 overflow-hidden ">
                <h1 class="text-2xl font-bold text-wrap">{{$product->name}}</h1>
                <p class="text-justify text-orange-500 text-xl font-bold">Rp {{$product->price}}</p>
                <p>Stok : {{$product->stock}}</p>
                <p>Kategori : {{$product->category->category_name}}</p>
                <div class="">
                    <p class="font-bold">Deskripsi : </p>
                    <div class="h-28 overflow-y-auto scrollbar-hide">
        ...
        <div class="flex justify-end px-5">
            <button class="bg-orange-500 px-3 py-2 rounded-lg text-white">Beli Sekarang</button>
        </div>
    </main>
